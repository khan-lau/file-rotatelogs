...
func New(p string, options ...Option) (*RotateLogs, error) {
	...
	var namingFunc NamingFunc // 自定义滚动命名规则回调
	var agingFunc AgingFunc   // 自定义老化清理回调
	...
	for _, o := range options {
		...
		case optkeyForceNewFile:
			forceNewFile = true
		case optkeyNamingFunc:
			namingFunc = o.Value().(NamingFunc) // 自定义滚动命名规则
		case optkeyAgingFunc:
			agingFunc = o.Value().(AgingFunc) // 自定义老化清理逻辑
		}
	}
	...
	// 当既未设置 maxAge/rotationCount，也未设置自定义老化回调时，默认 maxAge 为 7 天
	if maxAge == 0 && rotationCount == 0 && agingFunc == nil {
	...
	return &RotateLogs{
		...
		namingFunc:    namingFunc, // 自定义滚动命名规则
		agingFunc:     agingFunc,  // 自定义老化清理逻辑
	}, nil
}
...
func (rl *RotateLogs) getWriter_nolock(bailOnRotateFail, useGenerationalNames bool) (io.Writer, error) {
	...
	if forceNewFile {
		...
		for {
			if generation == 0 {
				name = filename
			} else if rl.namingFunc != nil {
				// 调用用户自定义命名规则：baseFilename 和 generation 由参数传入
				name = rl.namingFunc(filename, generation)
			} else {
				// // 有缺陷
				// name = fmt.Sprintf("%s.%d", filename, generation)

				// 改为：将代际后缀插入到扩展名之前，避免 *glob 不匹配
				if ext := filepath.Ext(filename); ext != "" {
					base := filename[:len(filename)-len(ext)]
					name = fmt.Sprintf("%s.%d%s", base, generation, ext)
				} else {
					// 无扩展名时保持原样（如 app.20260721.1）
					name = fmt.Sprintf("%s.%d", filename, generation)
				}
			}
...
func (rl *RotateLogs) rotate_nolock(filename string) error {
	...
	// 匹配旧版代际后缀文件 *.log.1, *.log.2, ...
	gmatches, err := filepath.Glob(rl.globPattern + ".*")
	if err == nil {
		matches = append(matches, gmatches...)
	}

	// 匹配新版代际后缀文件 *.1.log, *.2.log, ...
	if ext := filepath.Ext(rl.globPattern); ext != "" {
		base := rl.globPattern[:len(rl.globPattern)-len(ext)]
		ngmatches, err := filepath.Glob(base + ".*" + ext)
		if err == nil {
			matches = append(matches, ngmatches...)
		}
	}
	...
	if rl.agingFunc != nil {
		...
		for _, path := range matches {
			...
			fileInfos = append(fileInfos, LogFileInfo{Path: path, FileInfo: fi})
		}
